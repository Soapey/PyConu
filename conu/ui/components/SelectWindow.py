...
class SelectWindow(QMainWindow):
    ...
    def __init__(self, selecting_type: type, set_value_func, entity_attribute_name_to_set_value, set_property_func, headers_dict: dict) -> None:
        super().__init__()
        self.selecting_type = selecting_type
    ...
    def _select(self):
        ...
        set_value_value = getattr(selected_entity, self.entity_attribute_name_to_set_value)
    ...
    def _entities_by_search(self, search_text: str):
        ...
        self.entities = select_by_attrs_dict(self.selecting_type)

        if not search_text:
            matches = list(self.entities.values())
        else:
            matches = [e for e in self.entities.values() if search_text in "".join([str(getattr(e, attr_name)).lower() for attr_name in self.headers_dict.keys()])]

        load_entities_into_table(self.ui.selectwindow_tblSelect, matches, self.headers_dict)

    def _connect_select_actions(self):
        self.ui.selectwindow_btnSelect.clicked.connect(lambda: self._select())
        self.ui.selectwindow_txtSearch.textChanged.connect(lambda: self._entities_by_search(self.ui.selectwindow_txtSearch.text().lower()))
